...
def read_ctgs_data(
    ...
    assembler=PANGENOME_TAG, gfa_gzipped=True,
    ...
    ctgs_cov_dict = read_GFA_normalized_coverage(
        in_gfa_file, gzipped=gfa_gzipped, cov_key=ASSEMBLER_COV_TAG[assembler]
    )
    ctgs_asspen_dict = read_GFA_ass_penalty(in_gfa_file, gzipped=gfa_gzipped) ## TODO Check this
...
def read_links_data(in_gfa_file, gfa_gzipped=True):
    ...
    links_list = []
    __links_dict = read_GFA_links(in_gfa_file, gzipped=gfa_gzipped)
...
def get_capacities(links_list, ctgs_data_dict):
    ...
    for link in links_list:
        ...
        capacity = min(
            ctgs_data_dict[ctg_from_id][COV_KEY], ctgs_data_dict[ctg_to_id][COV_KEY]
        )
