...
def get_plasmid_details(contigs_dict, filename, side, min_len):
	...
	for _, row in pls_ctg_df.iterrows():
		plasmid, contig, length = side+'_'+row['plasmid'], str(row['contig']), row['contig_len']
		if length >= min_len:
...
def comp_mode(left_plasmids_file, right_plasmids_file, p, min_len, max_calls, output_file, log_file):
	'''
	Reads input files
	...
	contigs_dict = {}
	pls_ids_dict = {'L': {}, 'R': {}}
	contigs_dict, pls_ids_dict['L'] = get_plasmid_details(contigs_dict, left_plasmids_file, 'L', min_len)
	contigs_dict, pls_ids_dict['R'] = get_plasmid_details(contigs_dict, right_plasmids_file, 'R', min_len)
	compare_sets.run_compare_plasmids(contigs_dict, pls_ids_dict, p, max_calls, results_file)