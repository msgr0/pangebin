...
def parser():
    ...
    parser.add_argument('--bin',
                        help='Input bin file, tsv output of plasbin flow', required=True)
    parser.add_argument('--sample',
                        help='Sample name', required=True)
    parser.add_argument('--csv',
                        help='Input csv file, ground truth file', required=True)
    parser.add_argument('--gfa',
                        help='Input gfa file containing the pangenome')
    parser.add_argument('--output',
                        help='Output folder')
    parser.add_argument('--plot',
                        help='Plot the results', action='store_true')
    parser.add_argument('--type',
                        help='Print the results', required=True, choices=['pangenome', 'skesa', 'unicycler', 'assembler', 'pangenome_no_penalty'])
...
def main():
    ...
    if broad_type == "pangenome":
        ...
        for seg in gfa.segments: # for each pangenome segment
            ...
            tlist = []
            for contig in contig_list:
                tlist.append(ground_truth.loc[ground_truth["contig"] == contig]['label'].values[0])
            if (len(set(tlist))) != 1:
                tlist = set(tlist)
                if "ambiguous" in tlist:
                    ...
                    for ct in contig_list:
                        ground_truth["label"] = ground_truth["label"].where(ground_truth["contig"].ne(ct), new_lab)
                else:
                    selected_id = ground_truth.loc[(ground_truth["contig"].isin(contig_list))]['length'].idxmax()
                    new_lab = ground_truth.at[selected_id, "label"]
                    for ct in contig_list:
                        ground_truth["label"] = ground_truth["label"].where(ground_truth["contig"].ne(ct), new_lab)
